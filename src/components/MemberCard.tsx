...
import { ConnectionQualityIndicator, ParticipantContext, ParticipantContextIfNeeded, 
    ParticipantName, TrackMutedIndicator, useConnectionQualityIndicator, AudioTrack } from "@livekit/components-react";
...
export default function MemberCard({ participant, isme, ...props }: AudioVisualizerProps) {
    ...
    return (
        ...
        <div className='p-2 rounded-xl sm:w-[150px]  text-white  animate__animated  animate__zoomIn' style={{ backgroundColor: theme.color1, boxShadow:"rgba(57, 108, 124, 0.5) 0px 6px 18px 0px"}}>
            {isMute || isme || <AudioTrack volume={undefined} source={Track.Source.Microphone} participant={participant}></AudioTrack>}
            
            <ParticipantContextIfNeeded  participant={participant}>
                <div className='flex justify-around break-words items-center'>
                    <ParticipantName />
                    {isme && <span>(ME)</span>}
                </div>
